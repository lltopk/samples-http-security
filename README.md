...
- 用户登录: 客户端用公钥对明文密码加密然后调用登录接口, 传输给后端, 服务端私钥解析密码得到`rawpassword`, 并用相同的哈希算法再次计算哈希`encode(rawpassword, salt)`, 与数据库密码`encodePwd`对比, 一致则通过认证

注意, 注册用户最好不要使用MD5存储密码, MD5已知存在彩虹表攻击(预先计算好的海量"明文-哈希值"对应表)和哈希碰撞(不同内容相同MD5容易伪造内容). 

MD5的优点是快速, 因此是为了校验数据完整性设计的，而不是用于密码存储。

在处理海量数据时，直接对比两个长文本或大文件是非常消耗性能的。MD5 可以将任意长度的数据（无论是 10MB 的文章还是 1GB 的视频）压缩成固定 32 位的字符串。
- 比如文章系统，计算文章内容的 MD5。如果数据库里已经存在这个 MD5 值，就可以判定内容重复，无需再次存储。
- 比如在 Redis 或 Memcached 等缓存系统中，Key 的长度直接影响内存占用和网络传输效率。可以直接把这个复杂的对象或长 URL 进行 MD5 哈希，生成一个固定长度的 32 位字符串作为缓存的 Key。这样既保证了唯一性，又节省了内存空间。
- 比如在分布式架构中，如何生成全局唯一的 ID。可以将"时间戳 + 机器 IP + 进程 ID + 随机数"组合起来，进行一次 MD5 运算，截取一部分作为唯一标识符（UUID 的某些实现版本就利用了 MD5 的思想）。虽然 UUID 标准实现不完全是 MD5，但原理类似，都是利用哈希算法生成唯一指纹。
- 比如在 Android 或 iOS 应用开发中，MD5 常被用来做轻量级的自我检查。 App 启动时，计算自身核心库文件或配置文件的 MD5，与打包时记录的值对比。如果不同，说明文件可能被破坏或被用户修改（破解），App 可以选择拒绝运行或重新下载资源。
